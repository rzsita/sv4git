...
The `monorepo-*` commands let each component maintain its own semver in a YAML/JSON file. All logic is in `sv/monorepo.go` and `cmd/git-sv/handlers.go`.
...
`path` is parsed by `parsePath()` in `sv/monorepo.go`. Bracket notation handles keys with dots/special chars. Leading `.` is optional.
...
`componentCommits` is correct for `monorepo-tag` because it always tags immediately, so the on-disk version never leads committed state. The others need `componentBaseVersionAndCommits` for idempotency.
